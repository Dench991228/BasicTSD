...
def masked_decentral_mae(prediction: torch.Tensor, target: torch.Tensor, null_val: float = np.nan) -> torch.Tensor:
    """
    ...
    prediction_t_mean = prediction_t_mean.unsqueeze(1)
    target_t_mean = torch.mean(target * mask, dim=1)
    target_t_mean = target_t_mean.unsqueeze(1)
    return masked_mae(prediction - prediction_t_mean, target-target_t_mean, null_val=null_val)