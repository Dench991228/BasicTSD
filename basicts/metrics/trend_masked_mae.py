...
def trend_masked_mae(prediction: torch.Tensor,
                     target: torch.Tensor,
                     null_val: float = np.nan,
                     prediction_trend: torch.Tensor = None,
                     target_trend: torch.Tensor = None) -> torch.Tensor:
    """
...
def seasonal_masked_mae(prediction: torch.Tensor,
    ...
    loss = torch.abs((prediction - prediction_trend) - (target - target_trend))
    loss = loss * mask  # Apply the mask to the loss
    loss = torch.nan_to_num(loss)  # Replace any NaNs in the loss with zero

    return torch.mean(loss)

def trend_only_masked_mae(prediction: torch.Tensor,
                     target: torch.Tensor,
                     null_val: float = np.nan,
                     prediction_trend: torch.Tensor = None,
                     target_trend: torch.Tensor = None) -> torch.Tensor:
    ...
    loss = torch.abs((prediction_trend) - (target))
    loss = loss * mask  # Apply the mask to the loss
    loss = torch.nan_to_num(loss)  # Replace any NaNs in the loss with zero

    return torch.mean(loss)

def middle_prediction_mae(prediction: torch.Tensor,
                     target: torch.Tensor,
                     null_val: float = np.nan,
                     prediction_middle: torch.Tensor = None,
                     ) -> torch.Tensor:
    """
    Calculate the Masked Mean Absolute Error (MAE) between the predicted and target values,
    while ignoring the entries in the target tensor that match the specified null value.
    ...
    loss = torch.abs((prediction_middle - target))
    loss = loss * mask  # Apply the mask to the loss
    loss = torch.nan_to_num(loss)  # Replace any NaNs in the loss with zero
