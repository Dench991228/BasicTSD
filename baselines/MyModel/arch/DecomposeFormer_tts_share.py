...
class DecomposeFormer_TTS_Share(nn.Module):
    """
    ...
    def __init__(
        ...
        self.trend_embedding = Embedder(num_nodes, in_steps, input_dim, input_embedding_dim, steps_per_day,
                                        tod_embedding_dim,
                                        dow_embedding_dim, spatial_embedding_dim, 0)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int,
        ...
        # note 这里进行embedding，这一版本模型共享adaptive embedding
        ...
        if self.adaptive_embedding_dim > 0:
            adp_emb = self.adaptive_embedding.expand(
                size=(bs, *self.adaptive_embedding.shape)
            )
            trend_embeddings = torch.cat([trend_embeddings, adp_emb], dim=-1)
            seasonal_embeddings = torch.cat([seasonal_embeddings, adp_emb], dim=-1)
        ...
        # note 整理一下需要输出的表示
        # (batch_size, num_nodes, in_steps, model_dim * 2)
