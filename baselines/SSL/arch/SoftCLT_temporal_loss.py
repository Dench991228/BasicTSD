...
class SoftCLTLoss_global_t(SoftCLT_Loss):
    ...
    def forward(self,
        ...
        # note 先计算一下全局特征之间的距离
        # (B, D)
        global_repr = torch.mean(original_feature, dim=1).clone().detach()
        global_repr.requires_grad = False
        # note 首先计算两两特征之间的距离
        # note 整理一下特征
        batch_size, nodes, dims = original_feature.shape
        _, _, T = original_series.shape
...
class SoftCLTLoss_tod(SoftCLT_Loss):
    def forward(self,
                original_feature: torch.Tensor,
                augmented_feature: torch.Tensor,
                original_series: torch.Tensor,
        ...
        b, t, n = original_series.shape
        # (b, n, t) -> (b*n, t)
        original_series = original_series.permute((0, 2, 1)).contiguous()
        original_series = original_series.reshape(b*n, t)
        print("permuted series shape", original_series.shape)
        # note 整理一下特征
        batch_size, nodes, dims = original_feature.shape
        # (bs * n, d)
        original_feature = original_feature.reshape(batch_size * nodes, dims)
        augmented_feature = augmented_feature.reshape(batch_size * nodes, dims)
        ...
        if not self.hard: # 如果是软对比学习，需要增加对原始数据的计算
            ...
            # note 计算tod部分的差值
            # (B) tod / max_tod
            assert 'tod' in kwargs and isinstance(kwargs['tod'], torch.Tensor)
            tod = kwargs['tod']*torch.pi*2
            # (B,) -> (B, 1) -> (B, N) -> (B*N)
            tod = tod.unsqueeze(-1).repeat(1, nodes).reshape(-1)
            # (B*N, B*N) -> (2B*N, 2B*N)
            tod_dist = torch.cos(torch.abs(tod.unsqueeze(-1) - tod.unsqueeze(0))).repeat((2, 2))
            print("tod dist", torch.mean(2-tod_dist))
            pairwise_dist *= (2 - tod_dist)
            print("mean dist after tod", torch.mean(pairwise_dist))
            print("shape of pairwise dist", pairwise_dist.shape)
            soft_label = 2 * self.alpha*F.sigmoid(-self.tau * pairwise_dist)
            # 接下来去掉对角线 (2*B*N, 2*B*N-1)
